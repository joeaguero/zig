...
pub fn build(b: *std.Build) void {
    ...
    const run_chmod_step = if (host.result.isDarwin()) steps: {
        ...
        run_chmod.has_side_effects = true;
    ...
    const build_cc_stage2_step = if (b.option(
        ...
        build_stage3.has_side_effects = true;
...

    {
        ...
        if (b.option([]const u8, "maxrss", "Limit memory usage")) |maxrss|
            run_tests.addArgs(&.{ "$--maxrss", maxrss });
        ...
        for (b.option([]const []const u8, "extra-target", "Extra targets to build") orelse &.{}) |build_target| {
            ...
            build_stage4.has_side_effects = true;
        ...
        switch (build_type) {
            ...
            .Release => {
                ...
                const build_cmp = b.addExecutable(.{
                    .name = "cmp",
                ...
                run_cmp.addArtifactArg(build_cmp);
        ...
        if (binaryen_lazy_dep) |binaryen_dep| {
            ...
            update_zig1.has_side_effects = true;
            ...
            build_updated_stage4.has_side_effects = true;

            const run_test_with_updated_stage4 = std.Build.Step.Run.create(b, "test updated cmake bootstrap (stage3)");
            run_test_with_updated_stage4.addFileArg(updated_stage4_prefix.path(b, b.fmt("bin/zig{s}", .{host_exe_file_ext})));
            run_test_with_updated_stage4.addArg("test");
            run_test_with_updated_stage4.addFileArg(b.path("../test/behavior.zig"));
            run_test_with_updated_stage4.has_side_effects = true;
            update_stage1_step.dependOn(&run_test_with_updated_stage4.step);
        }
    }
}
